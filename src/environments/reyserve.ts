    ...
    return build(opts, config)
        ...
        .then(() => {
          ...
          REYSERVE_FILES.forEach((file) => {
            fs.copySync(
                path.join(reyserveFiles, file), path.join(bundled, file));
            fs.copySync(
                path.join(reyserveFiles, file), path.join(unbundled, file));
          });
        });
  }
  ...
  serve(options: ServerOptions): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      let devServer = child_process.spawn(
          'dev_appserver.py',
          ['app.yaml'],
          {stdio: 'inherit', cwd: options.root});
