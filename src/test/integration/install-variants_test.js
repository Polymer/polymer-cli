...
suite('install-variants', function() {
  ...
  test('installs variants', (done) => {
    const fixturePath = path.join(__dirname, '../../../test-fixtures/install-variants');
    ...
    tmp.dir((err, tmpPath, cleanup) => {
      fs.copy(fixturePath, tmpPath, {}, (err) => {
        ...
        runCommand(
            binPath,
            // NOTE(aomarks) Bower will fall back to your ~/.cache, which could
            // cause issues with this test working locally but not on travis
            // or another machine. Is there a way to force it to only read
            // from our cache on disk?
